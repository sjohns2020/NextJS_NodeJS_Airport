...
const FilterBar = ({ uniqueAirlines, setDisplayedFlights, handleSearch }) => {
  ...
  const airlines = uniqueAirlines.map((flight) => {
    ...
    return (
      <img
        ...
        className={
          searchTerm === activeStatus ? 'active common-size' : 'common-size'
        }
      />
    );
  });
  ...
  return (
    <div className="filter-container">
          ...
          <div
            className={
              status.value === activeStatus
                ? 'filter-button active common-size'
                : 'filter-button common-size'
            }
            key={status.value}
          >
            <button
      ...
      <div className="airport-images">
        <button
          className="filter-button reset common-size"
          onClick={() => {
            handleSearch();
          }}
        >
